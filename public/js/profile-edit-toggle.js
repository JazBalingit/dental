...
// data-edit-toggle> starts with its [data-editable] fields disabled; its
// data-edit-btn="edit" button enables them and swaps in the
// data-edit-btn="cancel"/"save" buttons. Cancel reverts to the
// server-rendered values and re-locks the form. The actual "are you sure"
// confirmation on save comes from partials/confirm-action-modal.blade.php's
// shared data-confirm-message form handling — this script only owns the
// lock/unlock state, not the submit-time confirmation.
(function () {
    function setEditing(form, fields, buttons, on) {
    ...
    function bind(form) {
        ...
        buttons.edit.addEventListener('click', function () {
            setEditing(form, fields, buttons, true);
        });
        ...
        if (buttons.cancel) {
            buttons.cancel.addEventListener('click', function () {
                ...
                setEditing(form, fields, buttons, false);
            });
        }
