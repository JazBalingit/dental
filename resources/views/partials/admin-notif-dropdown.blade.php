...
<div class="dropdown">
    <button class="icon-btn" type="button" data-bs-toggle="dropdown" aria-expanded="false">
        <i class="bi bi-bell"></i>@if($adminUnreadCount > 0)<span class="dot">{{ $adminUnreadCount }}</span>@endif
    </button>
    <ul class="dropdown-menu dropdown-menu-end notif-dropdown shadow-sm p-2"
            ...
            <li>
                <form method="POST" action="{{ route('notifications.read', $n->NotificationID) }}" class="m-0">
                    ...
                    <button type="submit"
                        ...
                        style="{{ !$n->IsRead ? 'background:var(--brand-50);font-weight:600;' : '' }}">
        ...
        @empty
            <li>
                <p class="text-center text-muted-2 small mb-0 py-3">No notifications yet.</p>
            </li>
        @endforelse
